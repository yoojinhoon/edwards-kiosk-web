...
bx.Loader = {
  ...
  $7C: function () {
    ...
    bx.Trace.error = {
      warning: { $AZ: 2001, $B0: '' },
      warnProperty: { $AZ: 2002, $B0: 'Property is not properly setup' },
      warnParameter: { $AZ: 2003, $B0: 'Function parameter error' },
      warnType: { $AZ: 2004, $B0: 'Type error' },
      warnClassDefine: { $AZ: 2101, $B0: 'A class tried to inherit twice' },
      error: { $AZ: 5001, $B0: '' },
      errorParameter: { $AZ: 5002, $B0: 'Function parameter error' },
      invalidClass: { $AZ: 5003, $B0: 'Invalid class was tried to instantiate' },
      errorDOM: { $AZ: 5004, $B0: 'DOM error!' },
      exception: { $AZ: 5005, $B0: 'Exception!' },
      errorJSON: { $AZ: 5006, $B0: 'JSON parsing error' },
      errorXHR: { $AZ: 5007, $B0: 'XHR error' },
      noProperty: { $AZ: 5008, $B0: 'Property is not properly setup' },
      errorDevice: { $AZ: 5009, $B0: 'Device error!' },
      errorPosition: { $AZ: 5101, $B0: 'Error in CPosition' },
      errorNoElement: { $AZ: 5102, $B0: "HTML Element doesn't exist" },
      errorImport: { $AZ: 5103, $B0: 'Module import is failed' },
    };
    bx.HCL.init();
    bx.DOM.init();
    bx.EventManager.$5X();
    ...
    if (bx.Loader.$DF.onRun) {
      ...
      if (window.onBxRun) {
        ...
        bx.Trace.log(
          'BXG',
          'warning',
          'BLUX onBxRun() or custom run() function is missed.',
          'bx.Loader.load',
        );
        return;
      }
...
bx.$50.XHR = { stateWait: 3, stateDone: 4, stateError: 5 };
...
bx.$50.importModule = function (moduleURL, forceProto, noCaching, onLoad, onError, _noMultiCheck) {
  function check(url) {
    if (!_noMultiCheck) {
      if (bx.$50.$5V[url]) {
        ...
        bx.$50.$5V[url] = { status: 1, loads: [], errors: [] };
        if (onLoad) {
          bx.$50.$5V[url].loads.push(onLoad);
        }
        if (onError) {
  ...
  if (forceProto == 2) {
    ...
    if (scriptList.length > 0) {
      ...
      scriptList[scriptList.length - 1].parentNode.insertBefore(
        importTag,
        scriptList[scriptList.length - 1].nextSibling,
      );
    }
  } else {
...
bx.$applyMsFilter = function (c, d, g) {
  ...
  if (c.bxMsFilterAlpha !== undefined && c.bxMsFilterAlpha != 1) {
    e +=
      ...
      parseInt(parseFloat(c.bxMsFilterAlpha) * 100) +
      ')';
  }
  return e;
};
bx.HCL = {
  ...
  $0e: function (e, d, h) {
    ...
    if (bx.HCL.DV.supportCSS3() && this.$0f[d]) {
      ...
      if (f) {
        g = { style: f + d.charAt(0).toUpperCase() + d.substring(1), value: h };
      }
    } else {
      if (d == 'cssFloat') {
        g = { style: 'styleFloat', value: h };
      } else {
        if (d == 'styleFloat') {
          g = { style: 'cssFloat', value: h };
        } else {
          if (d == 'opacity') {
            g = { style: 'filter', value: bx.$applyMsFilter(e, 'opacity', h) };
          } else {
            if (d == 'filter' && h.indexOf('opacity') != -1) {
              var c = h.search(/[0-9]/);
              if (c != -1) {
                g = { style: 'opacity', value: parseInt(h.substring(c)) / 100 + '' };
              }
            }
          }
        }
  ...
  findAvailCssSheet: function () {
    if (!document.styleSheets || !document.styleSheets.length) {
      ...
      for (var d = 0; d < document.styleSheets.length; d++) {
        e = undefined;
        try {
          e = document.styleSheets[d].cssRules
            ? document.styleSheets[d].cssRules
            : document.styleSheets[d].rules;
        } catch (c) {}
        if (!e) {
          continue;
        } else {
  ...
  $07: function () {
    return {
      ...
      y: isNaN(parseInt(document.body.style.marginTop))
        ? 0
        : parseInt(document.body.style.marginTop),
    };
  },
  $08: function () {
  ...
  getElementRect: function (g, f) {
    ...
    if (
      g.offsetWidth == 0 &&
      g.offsetHeight == 0 &&
      !(g.style.width == 'auto' && g.style.height == 'auto')
    ) {
      bx.Trace.log(
        'BLUX',
        'warning',
        'Try to get thie size of invisible Tag or Size=0 Tag. id=' + g.id,
        'HCL.getElementRect',
      );
    }
    f.x = bx.HCL.$0C(g) + d;
    f.y = bx.HCL.$0D(g) + c;
    f.w =
      (g.$css('box-sizing') == 'border-box' ? g.clientWidth : g.offsetWidth) ||
      bx.$checkNaN(parseInt(g.$css('width'))) ||
      bx.$checkNaN(parseInt(g.style.width));
    f.h =
      (g.$css('box-sizing') == 'border-box' ? g.clientHeight : g.offsetHeight) ||
      bx.$checkNaN(parseInt(g.$css('height'))) ||
      bx.$checkNaN(parseInt(g.style.height));
    return f;
  },
  ...
  DV: {
    ...
    getBrowser: function () {
      return { type: bx.$50.$0a[1], ver1: bx.$50.$0a[3], ver2: bx.$50.$0a[4], ver3: bx.$50.$0a[5] };
    },
...
bx.Event = {
  ...
  isLeftButton: function (c) {
    if (
      c.type == 'mousedown' &&
      !(c.which == 1 || c.button == 0 || (bx.$50.$0a[1] == 'msie' && c.button == 1))
    ) {
      return false;
    }
    return true;
  ...
  init: function () {
    ...
    if (window.Element) {
      ...
      bx.$apply3D = function (c) {
        c.$CSS(
          'transform',
          (c.bxTrX !== undefined || c.bxTrY !== undefined
            ? 'translate3d(' +
              bx.$checkNaN(parseInt(c.bxTrX)) +
              'px,' +
              bx.$checkNaN(parseInt(c.bxTrY)) +
              'px,0)'
            : '') +
            (c.bxTrScale3D ? c.bxTrScale3D + ' ' : '') +
            (c.bxTrRotateZ ? c.bxTrRotateZ + ' ' : ''),
        );
      };
...
bx.UXmanager = {
  ...
  init: function (e) {
    ...
    if (bx.CWnd) {
      this.$2G = { popup: 910000, waiting: 920000, alert: 930000 };
      if (bx.HCL.DV.hasTouchEvent() && window.bxg) {
        this.$DF.type = this.$DF.type
          ? this.$DF.type | bx.UX.SYS_USE_TOUCHEND
          : bx.UX.SYS_USE_TOUCHEND;
      }
      if (this.$DF.debug) {
        this.$DF.type = this.$DF.type
          ? this.$DF.type | bx.UX.SYS_CONTEXT_MENU
          : bx.UX.SYS_CONTEXT_MENU;
        bx.Log.init(true);
    ...
    if (this.$DF.rootTag && this.$DF.resize) {
      ...
      bx.Trace.log(
        'BLUX',
        'warning',
        'resize option is not allowed when rootTag option exists. So, resize option is ignored.',
        'UX.init',
      );
    }
    var d = bx.$50.$0a[1];
    ...
    if (!(this.$DF.type & bx.UX.SYS_NO_VIEWPORT) && !this.$DF.resizeSimple) {
      if (d == 'iphone' && parseInt(bx.HCL.$0a[3]) >= 11) {
      } else {
        if (bx.HCL.DV.isIOS()) {
            ...
            if (window.bluega && window.bluega.ios) {
              bx.$setMeta(
                ...
                'width=640; height=' +
                  ...
                  '; user-scalable=0; initial-scale=0.5; minimum-scale=0.5; maximum-scale=0.5;',
              );
            } else {
              ...
              if (this.$DF.type & bx.UX.SYS_NO_ADDR_BAR) {
                ...
                bx.$setMeta(
                  'viewport',
                  'width=' +
                    bx.HCL.getViewportWidth() * 2 +
                    '; height=' +
                    bx.HCL.getViewportHeight() * 2 +
                    '; user-scalable=0; initial-scale=0.5; minimum-scale=0.5; maximum-scale=0.5;',
                );
              }
            }
          ...
          if (bx.HCL.DV.isTizen() && window.devicePixelRatio && window.devicePixelRatio == 2) {
            bx.$setMeta(
              'viewport',
              'initial-scale=0.5, maximum-scale=0.5, minimum-scale=0.5, user-scalable=0, width=device-width, height=device-height',
            );
          }
        }
      }
    ...
    if (bx.CWnd) {
      ...
      this.$52 = new bx.CPane(
        null,
        'bxWndRoot',
        new bx.CPosition({ type: 'ondeck', width: '100%', height: '100%' }),
      );
      this.$52.create();
      this.domBuffer = this.$52.tag.$TAG('div', { id: 'bx$SYS_domBuffer' });
      this.DOMbuffer = this.domBuffer;
      this.domBuffer.$CSS({ display: 'none' });
    }
    if (this.$DF.resize) {
    ...
    if (!(this.$DF.type & bx.UX.SYS_HIGHLIGHT) && bx.HCL.DV.supportCSS3()) {
      bx.HCL.addCssRule(
        'html, body, div, dl, form, ol, p, table, ul, dd, dt, li, tbody, td, tfoot, th, thead, tr, a, b, br, i, img, small, span, strong, sub, sup, legend, fieldset, input, map, area, textarea',
        '-webkit-tap-highlight-color: rgba(0,0,0,0.0);-o-tap-highlight-color: rgba(0,0,0,0.0);-moz-tap-highlight-color: rgba(0,0,0,0.0);-ms-tap-highlight-color: rgba(0,0,0,0.0);tap-highlight-color: rgba(0,0,0,0.0);',
      );
    }
    if (!(this.$DF.type & bx.UX.SYS_SELECTABLE) && bx.HCL.DV.supportCSS3()) {
      if (d == 'firefox' || d == 'mozilla') {
        bx.HCL.addCssRule(
          'html, body, div, dl, ol, p, table, ul, dd, dt, li, tbody, td, tfoot, th, thead, tr, a, b, br, i, img, small, span, strong, sub, sup, legend, map',
  ...
  onResizeRootTag: function (c) {
    ...
    if (bx.CWnd) {
      if (bx.UX.$2P) {
        bx.UX.$2P.$CSS({ width: bx.UX.width + 'px', height: bx.UX.height + 'px' });
      }
      bx.UX.$52.onResize({ x: 0, y: 0, w: bx.UX.width, h: bx.UX.height }, 1);
    }
  },
  $3W: function (h) {
    if (!h) {
    ...
    if (bx.CWnd) {
      if (
        bx.UX.$DF.resizeSimple &&
        h.type == 'orientationchange' &&
        bx.HCL.DV.isIOS() &&
...
bx.UX.cfg = {
  ...
  waitBoxSpinner: { frame: 1, size: 53, url: '' },
  modalBlockBase: null,
  cssMessage: 'bxUXmessage',
  cssModalBlock: 'bxScreenBlock',
  cssToast: 'bxToast',
};
bx.Log = { DEBUG: 1, WARNING: 2, ERROR: 3, $43: false };
bx.Log.cfg = {};
bx.Log.cfg.console = {
  right: '0px',
  bottom: '0px',
  ...
  if (this.$43) {
    ...
    if (this.$42 && this.$42.console) {
      if (!this.$44) {
        ...
        this.$44 = document.body.$TAG(
          'div',
          { align: 'left', onclick: 'this.style.display="none";' },
          this.cfg.console,
        );
      }
...
bx.PageTransition = function (i, j, k, p, c, n) {
  if (!i || !j || !k || !p) {
    bx.Trace.log(
      'BLUX',
      'errorParameter',
      'tagCnt=' + i + ', tagIn=' + j + ', tagOut=' + k + ', effect=' + p,
      'bx.PageTransition()',
    );
    return false;
  }
  ...
  if (!i.bxPageTransition.tagBlock) {
    i.bxPageTransition.tagBlock = i.$TAG('div', {
      style:
        'position:absolute;top:0px;left:0px;width:100%;height:100%;display:none;background-color:transparent;',
    });
  ...
  i.bxPageTransition.tagBlock.style.zIndex =
    Math.max(bx.$checkNaN(parseInt(j.$css('z-index'))), bx.$checkNaN(parseInt(k.$css('z-index')))) +
    1;
  i.bxPageTransition.tagPopup.style.zIndex =
    Math.min(bx.$checkNaN(parseInt(j.$css('z-index'))), bx.$checkNaN(parseInt(k.$css('z-index')))) +
    1;
  i.bxPageTransition.tagPopup.style.backgroundColor = 'transparent';
  if (g) {
    i.$CSS('perspective', i.offsetWidth + 'px');
    if (bx.HCL.DV.isIE()) {
      j.$CSS({ transformStyle: 'preserve-3d' });
      k.$CSS({ transformStyle: 'preserve-3d' });
    } else {
      j.$CSS({ transformStyle: 'preserve-3d', backfaceVisibility: 'hidden' });
      k.$CSS({ transformStyle: 'preserve-3d', backfaceVisibility: 'hidden' });
    }
  ...
  function u(v) {
    ...
    i.bxPageTransition.tagPopup.style.zIndex =
      Math.min(
        bx.$checkNaN(parseInt(j.$css('z-index'))),
        bx.$checkNaN(parseInt(k.$css('z-index'))),
      ) + 1;
    if (g) {
      i.$CSS('perspective', 'none');
      j.$CSS({ transformStyle: 'flat', transform: 'none' });
      k.$CSS({ transformStyle: 'flat', transform: 'none' });
    }
    j.$CSS({ transformOrigin: '50% 50% 0', animation: 'none', animationDelay: '0' });
    k.$CSS({ transformOrigin: '50% 50% 0', animation: 'none', animationDelay: '0' });
    if (s) {
      j.style.left = '0px';
      j.style.top = '0px';
      j.$CSS('opacity', 1);
  ...
  if (s) {
    if (p == 'none') {
      ...
      switch (p) {
        case 'moveLeft':
          t.styles = { left: { from: e, to: 0, unit: 'px' } };
          q.styles = { left: { from: 0, to: -e, unit: 'px' } };
          break;
        case 'moveRight':
          t.styles = { left: { from: -e, to: 0, unit: 'px' } };
          q.styles = { left: { from: 0, to: e, unit: 'px' } };
          break;
        case 'moveTop':
          t.styles = { top: { from: o, to: 0, unit: 'px' } };
          q.styles = { top: { from: 0, to: -o, unit: 'px' } };
          break;
        case 'moveBottom':
          t.styles = { top: { from: -o, to: 0, unit: 'px' } };
          q.styles = { top: { from: 0, to: o, unit: 'px' } };
          break;
        case 'dissolve':
          t.styles = { opacity: { from: 0, to: 1 } };
          q.styles = { opacity: { from: 1, to: 0 } };
          break;
      }
    ...
    switch (p) {
      ...
      case 'glueBottom':
        ...
        break;
      case 'fall':
        k.$CSS({ animation: 'rotateFall ' + c + 'ms both ease-in', transformOrigin: '0% 0%' });
        j.$CSS({ animation: 'scaleUp ' + Math.floor(c * 0.7) + 'ms ease both' });
        break;
      case 'moveLeft':
        k.$CSS({ animation: 'moveToLeft ' + c + 'ms ' + r + ' both' });
        h.animation = l('moveFromRight') + ' ' + c + 'ms ' + r + ' both';
        f();
        j.$CSS(h);
        break;
      case 'moveRight':
        k.$CSS({ animation: 'moveToRight ' + c + 'ms ' + r + ' both' });
        h.animation = l('moveFromLeft') + ' ' + c + 'ms ' + r + ' both';
        f();
        j.$CSS(h);
        break;
      case 'moveTop':
        k.$CSS({ animation: 'moveToTop ' + c + 'ms ' + r + ' both' });
        h.animation = l('moveFromBottom') + ' ' + c + 'ms ' + r + ' both';
        f();
        j.$CSS(h);
        break;
      case 'moveBottom':
        k.$CSS({ animation: 'moveToBottom ' + c + 'ms ' + r + ' both' });
        h.animation = l('moveFromTop') + ' ' + c + 'ms ' + r + ' both';
        f();
        j.$CSS(h);
        break;
      case 'bulldozeLeft':
        k.$CSS({ transformOrigin: '0% 50%', animation: 'squizToX ' + c + 'ms both ' + r + '' });
        j.$CSS({
          transformOrigin: '100% 50%',
        ...
        break;
      case 'bulldozeRight':
        k.$CSS({ transformOrigin: '100% 50%', animation: 'squizToX ' + c + 'ms both ' + r + '' });
        j.$CSS({ transformOrigin: '0% 50%', animation: 'stretchFromX ' + c + 'ms both ' + r + '' });
        break;
      case 'bulldozeTop':
        k.$CSS({ transformOrigin: '50% 0%', animation: 'squizToY ' + c + 'ms both ' + r + '' });
        j.$CSS({
          transformOrigin: '50% 100%',
        ...
        break;
      case 'bulldozeBottom':
        k.$CSS({ transformOrigin: '50% 100%', animation: 'squizToY ' + c + 'ms both ' + r + '' });
        j.$CSS({ transformOrigin: '50% 0%', animation: 'stretchFromY ' + c + 'ms both ' + r + '' });
        break;
      case 'slideUpLeft':
        k.$CSS({ animation: 'rotateSlideOutToLeft ' + c + 'ms both ' + r + '' });
        j.$CSS({ animation: 'rotateSlideInFromRight ' + c + 'ms both ' + r + '' });
        break;
      case 'slideUpRight':
        k.$CSS({ animation: 'rotateSlideOutToRight ' + c + 'ms both ' + r + '' });
        j.$CSS({ animation: 'rotateSlideInFromLeft ' + c + 'ms both ' + r + '' });
        break;
      case 'slideUpTop':
        k.$CSS({ animation: 'rotateSlideOutToTop ' + c + 'ms both ' + r + '' });
        j.$CSS({ animation: 'rotateSlideInFromBottom ' + c + 'ms both ' + r + '' });
        break;
      case 'slideUpBottom':
        k.$CSS({ animation: 'rotateSlideOutToBottom ' + c + 'ms both ' + r + '' });
        j.$CSS({ animation: 'rotateSlideInFromTop ' + c + 'ms both ' + r + '' });
        break;
      case 'cardUpLeft':
        k.$CSS({ animation: 'moveToLeft ' + c + 'ms ' + r + ' both' });
        j.$CSS({ animation: 'scaleUp ' + Math.floor(c * 0.85) + 'ms ' + r + ' both' });
        break;
      case 'cardUpRight':
        k.$CSS({ animation: 'moveToRight ' + c + 'ms ' + r + ' both' });
        j.$CSS({ animation: 'scaleUp ' + Math.floor(c * 0.85) + 'ms ' + r + ' both' });
      ...
      case 'cardUpBottom':
        ...
        break;
      case 'timeLagLeft':
        k.$CSS({ animation: 'moveToLeft ' + c + 'ms ease-in-out both' });
        h.animation = l('moveFromRight') + ' ' + Math.floor(c * 0.85) + 'ms ease both';
        f();
        j.$CSS(h);
        break;
      case 'timeLagRight':
        k.$CSS({ animation: 'moveToRight ' + c + 'ms ease-in-out both' });
        h.animation = l('moveFromLeft') + ' ' + Math.floor(c * 0.85) + 'ms ease both';
        f();
        j.$CSS(h);
        break;
      case 'timeLagTop':
        k.$CSS({ animation: 'moveToTop ' + c + 'ms ease-in-out both' });
        h.animation = l('moveFromBottom') + ' ' + Math.floor(c * 0.85) + 'ms ease both';
        f();
        j.$CSS(h);
        break;
      case 'timeLagBottom':
        k.$CSS({ animation: 'moveToBottom ' + c + 'ms ease-in-out both' });
      ...
      case 'cubeRight':
        ...
        j.$CSS({
          transformOrigin: '100% 50%',
          animation:
            'rotateCubeRightIn' + (bx.HCL.DV.isIE() ? 'IE' : '') + ' ' + c + 'ms both ease',
        });
        break;
      case 'cubeTop':
        k.$CSS({
      ...
      case 'cubeBottom':
        ...
        j.$CSS({
          transformOrigin: '50% 100%',
          animation:
            'rotateCubeBottomIn' + (bx.HCL.DV.isIE() ? 'IE' : '') + ' ' + c + 'ms both ease',
        });
        break;
      case 'dissolve':
        k.$CSS({ animation: 'fadeOut ' + c + 'ms both ease' });
        j.$CSS({ animation: 'fadeIn ' + c + 'ms both ease' });
        break;
      case 'fadeOut':
        k.$CSS({
        ...
        j.$CSS({ animation: 'fadeIn ' + Math.floor(0.6 * c) + 'ms both ease-out' });
        break;
      case 'fadeIn':
        k.$CSS({ animation: 'fadeOut ' + Math.floor(0.6 * c) + 'ms both ease-out' });
        j.$CSS({
      ...
      case 'zoomOut':
        ...
        j.$CSS({ animation: 'idle ' + c + 'ms both ease' });
        break;
      case 'zoomIn':
        k.$CSS({ animation: 'idle ' + c + 'ms both ease' });
        j.$CSS({ animation: 'zoomFadeIn ' + c + 'ms both ease-out' });
        break;
      case 'newspaper':
      ...
      case 'none':
        if (c < 16) {
          ...
          k.$CSS({ animation: 'idle ' + c + 'ms both ease-out' });
          j.$CSS({ animation: 'idle ' + c + 'ms both ease-out' });
        }
        break;
    }
  }
...
bx.UX.messageBox = function (f, c) {
  ...
  if (bx.CMessageDialog) {
    if (!bx.UX.$5f[e]) {
      var d = { size: c.size, buttons: {}, CMessageDialog: { cssMessage: bx.UX.cfg.cssMessage } };
      if (c.layout) {
        d.layout = c.layout;
      }
      if (c.btnOK) {
        d.buttons.ok = { title: c.btnOK };
      }
      if (c.btnCancel) {
        d.buttons.cancel = { title: c.btnCancel };
      }
    ...
    bx.UX.$5f[e].doModal({ values: { icon: c.icon, message: f } });
  } else {
  }
};
bx.UX.showToast = function (d, c) {
  if (window.bluega && bluega.UTIL_showToastText) {
    ...
    if (!bx.UX.$35) {
      bx.UX.$35 = bx.UX.getRootTag().$TAG(
        'div',
        {},
        { position: 'absolute', maxWidth: parseInt(bx.UX.width * 0.75) + 'px' },
      );
    ...
    if (c && c.css) {
      bx.UX.$35.$A({ class: c.css });
    } else {
      if (bx.HCL.getCssRule('.' + bx.UX.cfg.cssToast)) {
        bx.UX.$35.$A({ class: bx.UX.cfg.cssToast });
      } else {
        bx.UX.$35.$CSS({
          padding: '10px',
...
bx.UX.$3N = function (d) {
  if (!this.$2P) {
    this.$2P = this.$52.tag.$TAG('div', { id: 'bx$SYS_modalBlock' });
    var c;
    if (bx.UX.cfg.modalBlockBase) {
      c = bx.HCL.getElementRect(bx.UX.cfg.modalBlockBase);
    } else {
      if (this.$DF.rootTag) {
        c = { w: bx.UX.width, h: bx.UX.height };
      } else {
        c = { w: bx.HCL.$09(), h: bx.HCL.$08() };
      }
    }
    this.$2P.$CSS({
  ...
  this.$2P.$A({ class: bx.UX.cfg.cssModalBlock });
  if (d) {
  ...
  this.$2P.$CSS({ zIndex: e, display: 'block' });
    ...
    if (e == '-') {
      if (d == 'width') {
        if (h.left) {
          c =
            f.w -
            bx.$checkNaN(parseInt(h.left)) -
            bx.$checkNaN(parseInt(g.substring(0, g.length - 1)));
        } else {
          c =
            f.w -
            bx.$checkNaN(parseInt(h.right)) -
            bx.$checkNaN(parseInt(g.substring(0, g.length - 1)));
        }
      } else {
        if (h.top) {
          c =
            f.h -
            bx.$checkNaN(parseInt(h.top)) -
            bx.$checkNaN(parseInt(g.substring(0, g.length - 1)));
        } else {
          c =
            f.h -
            bx.$checkNaN(parseInt(h.bottom)) -
            bx.$checkNaN(parseInt(g.substring(0, g.length - 1)));
        }
      }
      return c + 'px';
...
bx.CPosition.prototype.$3a = function (h, g) {
  ...
  if (typeof this.data.width != 'object') {
    this.data.width = { normal: this.data.width };
  }
  if (typeof this.data.height != 'object') {
    this.data.height = { normal: this.data.height };
  }
      ...
      if (this.pane.tag) {
        if (this.pane.tagVP) {
          if (this.$3G.width != 'auto') {
            this.pane.tagVP.$CSS({ width: '0px' });
          }
          if (this.$3G.height != 'auto') {
            this.pane.tagVP.$CSS({ height: '0px' });
          }
        }
        this.pane.tag.$CSS(this.$3G);
        this.setX(c);
        this.setY(j);
        this.rect = { x: c, y: j, w: f, h: d };
      }
    }
  } else {
...
bx.CMode.prototype.setChain = function (c) {
  this.chainDown = c;
  c.chainUp = this;
  c.wnd.tag.$CSS({ zIndex: bx.$checkNaN(parseInt(this.wnd.tag.$css('z-index'))) + 1 + '' });
};
...
bx.CModePopup.prototype.onPreShow = function () {
  ...
  if (!this.$3I) {
    for (var c = 0; c < this.opts.length; c++) {
      if (this.opts[c].timer) {
        this.$3J.push(
          bx.EM.add({ timer: { time: this.$5p[c].timer }, target: this, hdr: this.onEvent }),
        );
      } else {
        if (this.opts[c].event) {
          this.$3J.push(bx.EM.add({ event: this.$5p[c].event, target: this, hdr: this.onEvent }));
        } else {
          if (this.opts[c].type) {
  ...
  if (this.wnd && this.$D9 != 'normal') {
    ...
    this.wnd.tag.$CSS({ zIndex: this.$F6 + bx.UX.getZindex(this.$D9) + '' });
  }
  this.$3I = true;
  return ret;
};
bx.CModePopup.prototype.onPostHide = function (c) {
  ...
  if (this.wnd && this.$F6 && this.$D9 != 'normal') {
    this.wnd.tag.$CSS({ zIndex: this.$F6 });
  }
  this.$3I = false;
  return ret;
};
...
bx.CModeModal.prototype.onPreShow = function () {
  ...
  if (this.wnd) {
    ...
    this.wnd.tag.$CSS({ zIndex: bx.UX.getZindex('popup') });
  }
  ret = bx.CMode.prototype.onPreShow.call(this);
  return ret;
};
bx.CModeModal.prototype.onPostHide = function (c) {
  ...
  bx.UX.$2R();
  if (this.wnd && this.$2G) {
    this.wnd.tag.$CSS({ zIndex: this.$2G });
  }
  return ret;
};
...
bx.CPane.prototype.create = function () {
  ...
  this.onResize({ x: 0, y: 0, w: bx.UX.width, h: bx.UX.height }, 3);
  return this;
};
...
bx.CWnd = function (f, e, i, c, h, d) {
  if (!arguments || arguments.length > 0) {
    ...
    if (this.opts.titleBar) {
      ...
      if (
        this.opts.titleBar.css === undefined &&
        bx.HCL.getCssRule('.' + bx.CWnd.cfg.titleBar.css)
      ) {
        this.opts.titleBar.css = bx.CWnd.cfg.titleBar.css;
      }
      if (
        this.opts.titleBar.cssHover === undefined &&
        bx.HCL.getCssRule('.' + bx.CWnd.cfg.titleBar.cssHover)
      ) {
      ...
      if (this.opts.titleBar.btnClose && bx.CWnd.cfg.titleBar.btnClose) {
        if (bx.CWnd.cfg.titleBar.btnClose.normal) {
          this.opts.titleBar.btnClose.normal =
            this.opts.titleBar.btnClose.normal || bx.CWnd.cfg.titleBar.btnClose.normal;
        }
        if (bx.CWnd.cfg.titleBar.btnClose.hover) {
          this.opts.titleBar.btnClose.hover =
            this.opts.titleBar.btnClose.hover || bx.CWnd.cfg.titleBar.btnClose.hover;
        }
        if (bx.CWnd.cfg.titleBar.btnClose.css) {
          this.opts.titleBar.btnClose.css =
            this.opts.titleBar.btnClose.css || bx.CWnd.cfg.titleBar.btnClose.css;
        }
        if (bx.CWnd.cfg.titleBar.btnClose.onPostClick) {
          this.opts.titleBar.btnClose.onPostClick =
            this.opts.titleBar.btnClose.onPostClick || bx.CWnd.cfg.titleBar.btnClose.onPostClick;
        }
      }
...
bx.CWnd.cfg = {};
bx.CWnd.cfg.titleBar = { height: 46, css: 'bxCWnd_title', cssHover: 'bxCWnd_title_hover' };
bx.CWnd.TYPE_DRAG = 1;
bx.CWnd.TYPE_DROP = 2;
...
bx.CWnd.prototype.create = function (c, d) {
  ...
  if (!this.mode || !(this.mode instanceof bx.CMode)) {
    bx.Trace.log(
      'BLUX',
      'noProperty',
      'CWnd.mode is not properly set in ' + this.tagId,
      'CWnd.create',
    );
    return null;
  }
...
bx.CWnd.prototype.$F0 = function (c, e) {
  this.tag = document.body.$TAG('div', { id: this.tagId });
  if (this.opts.classNameWnd !== undefined) {
    this.tag.$A({ class: this.opts.classNameWnd });
  }
  if (this.opts.styles) {
  ...
  if (this.opts.mouseDrag) {
    if (bx.HCL.DV.hasTouchEvent()) {
      this.tag.$A({ ontouchstart: 'bx.Event.mouseDrag(this, event);' });
    } else {
      this.tag.$A({ onmousedown: 'bx.Event.mouseDrag(this, event);' });
    }
  }
  if (this.opts.type & bx.CWnd.TYPE_NO_VIEWPORT) {
    this.tagVP = this.tag;
  } else {
    this.tagVP = this.tag.$TAG(
      c,
      { id: this.tagId + bx.c.$2I },
      { overflow: 'hidden', position: 'relative', width: 'auto', height: 'auto' },
    );
  }
  if (!(this.opts.type & bx.CWnd.TYPE_NO_VIEWPORT)) {
    if (this.opts.className !== undefined) {
      this.tagVP.$A({ class: this.opts.className });
    }
  }
  var f = this;
  if (this.opts.titleBar) {
    this.$3L = this.tag.$TAG(
      'div',
      { id: this.tagId + '_bxTB', align: 'left' },
      {
        overflow: 'hidden',
        whiteSpace: 'nowrap',
    ...
    if (this.opts.titleBar.onLongClick || this.opts.titleBar.onPreLongClick) {
      ...
      d += '});';
      if (bx.HCL.DV.hasTouchEvent()) {
        this.$3L.$A({ ontouchstart: d });
      } else {
        this.$3L.$A({ onmousedown: d });
      }
...
bx.CWnd.prototype.move = function (c, e, d) {
  ...
  if (this.position.$DS(c, e)) {
    ...
    bx.Trace.log(
      'BLUX',
      'errorPosition',
      'CWnd:' + this.tagId + " doesn't have a movable CPosition",
      'CWnd.move',
    );
  }
};
...
bx.CWnd.prototype.onResize = function (j, i, e, f) {
  ...
  if (!(this.opts.type & bx.CWnd.TYPE_NO_VIEWPORT)) {
    ...
    if (this.tag.$css('height', true) != 'auto') {
      l =
        bx.$checkNaN(parseInt(this.tag.style.height)) -
        (bx.HCL.PR.$0T(this.tagVP.tagName) ? 0 : k.h) -
        g;
      this.tagVP.style.height = parseInt(this.opts.$38 * l) + 'px';
    }
    if (this.tag.$css('width', true) != 'auto') {
      d =
        bx.$checkNaN(parseInt(this.tag.style.width)) -
        (bx.HCL.PR.$0T(this.tagVP.tagName) ? 0 : k.w);
      this.tagVP.style.width = parseInt(this.opts.$38 * d) + 'px';
    }
  } else {
    var k = { left: 0, right: 0, top: 0, bottom: 0, w: 0, h: 0 };
  }
  ...
  if (g) {
    if (this.opts.titleBar.css) {
      this.$3L.$A({ class: this.opts.titleBar.css });
    }
  ...
  for (var d = 0; d < this.childWnds.length; d++) {
    if (!this.childWnds[d].$5b) {
      this.childWnds[d].onResize(
        { x: this.innerLeft, y: this.innerTop, w: this.innerWidth, h: this.innerHeight },
        e,
      );
    }
  }
...
bx.EM = {
  ...
  setFocus: function (c) {
    ...
    if (!bx.EM.$5u) {
      if (bx.HCL.DV.hasTouchEvent()) {
        bx.EM.$5u = bx.EM.add({ event: 'touchstart', callback: bx.EM.$5v });
      } else {
        bx.EM.$5u = bx.EM.add({ event: 'mousedown', callback: bx.EM.$5v });
      }
    }
  },
  getFocused: function () {
  ...
  setTimeout: function (c, d) {
    return bx.EM.add({ timer: { time: d }, destroy: true, callback: c });
  },
  setInterval: function (c, d) {
    return bx.EM.add({ timer: { time: d }, callback: c });
  },
};
...
bx.CTrace.prototype.log = function (c, h, f, e) {
  if (this.error[h]) {
    ...
    bx.Log.log(
      c + d + this.error[h].$AZ + ':' + this.error[h].$B0 + ' "' + f + '"' + (e ? ' in ' + e : ''),
      g,
    );
  }
};
      ...
      if (g.styles[d].sequence) {
        if (!g.styles[d].sequence.length) {
          bx.Trace.log(
            'BLUX',
            'warnProperty',
            'sequence of ' + d + ' is an empty Array',
            'CTransition.add',
          );
          delete g.styles[d];
          continue;
        ...
        if (g.styles[d].from === undefined) {
          g.styles[d].from = bx.$checkNaN(
            parseInt(
              g.element.$css(
                d.replace(/[A-Z]/g, function (i, j) {
    ...
    if (this.$49[this.$GY] && !this.$49[this.$GY].$4B) {
      ...
      if (c >= 0) {
        ...
        if (!this.$N1l.time) {
          ...
          if (this.$N1l.wait) {
            this.$N1m = bx.Easing.wait(
              this.$N1l.timing,
              this.$N1k - this.$N1l.$4A,
              0,
              1,
              this.$N1l.time,
              this.$N1l.roundUp,
              this.$N1l.wait,
              this.$N1n,
              this.$N1l.element && this.$N1l.element.apxTrCtxt
                ? this.$N1l.element.apxTrCtxt[this.$N1l.timing]
                : undefined,
            );
            if (!this.$N1l.roundUp) {
              if (this.$N1l.timing[0] == 'x') {
            ...
            if (!this.$N1l.roundUp) {
              this.$N1m = bx.Easing[this.$N1l.timing](
                this.$N1k - this.$N1l.$4A,
                0,
                1,
                this.$N1l.time,
          ...
          if (this.$N1l.styles[this.$N1n].sequence) {
            if ((this.$N1m == 1 && !this.$N1l.roundUp) || (this.$N1m == 0 && this.$N1l.roundUp)) {
              if (typeof this.$N1l.styles[this.$N1n].unit == 'function') {
                this.$N1l.styles[this.$N1n].unit(
                  this.$N1l.element,
                  this.$N1n,
                  this.$N1l.styles[this.$N1n].sequence[
                    this.$N1l.styles[this.$N1n].sequence.length - 1
                  ],
                  this.$N1l.styles[this.$N1n].sequence.length - 1,
                );
              } else {
                this.$N1l.element.$CSS(
                  this.$N1n,
                  this.$N1l.styles[this.$N1n].sequence[
                    this.$N1l.styles[this.$N1n].sequence.length - 1
                  ] + this.$N1l.styles[this.$N1n].unit,
              ...
              if (this.$N1l.styles[this.$N1n].sequence.length > this.$N1l.$53) {
                if (typeof this.$N1l.styles[this.$N1n].unit == 'function') {
                  this.$N1l.styles[this.$N1n].unit(
                    this.$N1l.element,
                    this.$N1n,
                    this.$N1l.styles[this.$N1n].sequence[this.$N1l.$53],
                    this.$N1l.$53,
                  );
                } else {
                  this.$N1l.element.$CSS(
            ...
            this.$N1B =
              this.$N1l.styles[this.$N1n].from +
              (this.$N1l.styles[this.$N1n].to - this.$N1l.styles[this.$N1n].from) * (this.$N1m / 1);
            if (typeof this.$N1l.styles[this.$N1n].unit == 'function') {
...
bx.Easing.wait = function (j, m, k, i, h, e, f, l, g) {
  if (f > 0 && f < 1) {
    if (m < Math.floor(h * f)) {
      ...
      if (e) {
        return bx.Easing.roundUp(
          j,
          Math.max(1, Math.min(Math.floor(m - h * f), Math.ceil(h * (1 - f)))),
          k,
          i,
          Math.ceil(h * (1 - f)),
        );
        ...
        return bx.Easing[j](
          Math.min(Math.floor(m - h * f), Math.ceil(h * (1 - f))),
          k,
          i,
          Math.ceil(h * (1 - f)),
          l,
...
bx.CPageManager = function (e, c, d) {
  ...
  bx.CWnd.call(
    this,
    e,
    'bx$PageManager_' + bx.CPageManager.$N1o,
    '',
    c ||
      new bx.CPosition({
        type: 'overlap',
...
bx.CPageManager.prototype.run = function (f, q, k) {
  ...
  if (q.transit && h && !bx.HCL.DV.supportCSS3()) {
    bx.Trace.log(
      'BLUX',
      'warning',
      'This browser does not support CSS3. Cannot run transit=' + q.transit,
      'CPageManager.run',
    );
    q.transit = null;
  }
    ...
    if (h) {
      ...
      if (
        !bx.PageTransition(
          ...
          q.transit,
          q.transitDuration,
          q.transitTiming ? { timing: q.transitTiming } : undefined,
        )
      ) {
        p();
        d();
...
bx.CPageManager.prototype.prev = function (d) {
  ...
  if (
    e !== undefined &&
    bx.PageTransition &&
    bx.PageTransition.isInTransit(this.get(e).tag.parentNode)
  ) {
    return false;
  }
...
bx.CGesture = function (q, p, d) {
      ...
      if (r.type == 'touchstart') {
        if (m.$N2y.pinchEvent) {
          ...
          if (
            r.touches.length > 1 &&
            r.targetTouches &&
            r.targetTouches[0].identifier !== undefined
          ) {
            return;
          } else {
            m.onEvent(m.tag, bx.CGesture.POINTER_END, 0, 0, 0, 0, r);
              ...
              if (
                ...
                if (!bx.CGesture._isProxyTag) {
                  bx.CGesture._isProxyTag = document.body.$TAG('input', {
                    ...
                    readonly: 'readonly',
                    style:
                      'position:absolute;left:-99999px;top:-99999px;width:1;heigth:1;opacity:0',
                  });
                }
  ...
  if (bx.HCL.DV.hasTouchEvent() || bx.HCL.DV.hasTouchAndMouseEvent()) {
    if (bx.CGesture.supportEventPassive()) {
      bx.Event.add(this.tag, 'touchstart', g, { capture: false, once: false, passive: true });
    } else {
...
bx.CGesture.$N0e = function (g, c, f, l) {
  ...
  function i(m) {
    ...
    if (
      m.changedTouches &&
      m.changedTouches[0] &&
      m.changedTouches[0].identifier !== undefined &&
      e !== undefined
    ) {
  ...
  if (!bx.HCL.DV.hasTouchEvent()) {
    if (bx.CGesture.preventDrag) {
      ...
      if (bx.HCL.DV.isIE()) {
        ...
        if (
          k.tagName &&
          ((k.tagName.toLowerCase() == 'input' && k.type != 'button') ||
            k.tagName.toLowerCase() == 'textarea' ||
            k.tagName.toLowerCase() == 'select')
        ) {
...
bx.CGesture.prototype.$DM = function (n, f) {
  ...
  if (l == bx.CGesture.POINTER_START) {
    ...
    if (l == bx.CGesture.POINTER_DRAG && this.$N0W) {
      if (
        Math.abs(c - this.$N0U) > bx.CGesture.OFFSET_TAP ||
        Math.abs(p - this.$N0V) > bx.CGesture.OFFSET_TAP
      ) {
        this.$N0W = false;
  ...
  if (
    this.$N0W &&
    l == bx.CGesture.POINTER_END &&
    (this.$N1s == bx.CGesture.POINTER_DRAG || this.$N1s == bx.CGesture.POINTER_START)
  ) {
    if (
      !(this.$N2y && this.$N2y.noLongholdEvent) &&
      this.$N1u &&
      n.timeStamp - this.$N1u > bx.CGesture.$N2z
    ) {
      if (!this.$N2K) {
        this.onEvent(
          this.tag,
          bx.CGesture.POINTER_HOLD,
          this.$N0U,
          this.$N0V,
          this.$N2H,
          this.$N2I,
      ...
      if (this.$N2y && this.$N2y.noDelayedEvent) {
        ...
        if (this.$N1v.use) {
          if (
            Math.abs(this.$N1v.x - this.$N0U) <= bx.CGesture.OFFSET_DBLTAP &&
            Math.abs(this.$N1v.y - this.$N0V) <= bx.CGesture.OFFSET_DBLTAP
          ) {
            this.onEvent(
              this.tag,
              bx.CGesture.POINTER_DBLTAP,
              this.$N1v.x,
              this.$N1v.y,
              this.$N1v.cx,
              this.$N1v.cy,
    ...
    if (!this.$N0W && l == bx.CGesture.POINTER_END && this.$N1s == bx.CGesture.POINTER_DRAG) {
      if (this.$N1v && this.$N1v.use) {
        ...
        if (this.$N1u && n.timeStamp - this.$N1u < bx.CGesture.$N2B) {
          if (
            Math.abs(c - this.$N0U) > j &&
            Math.abs(p - this.$N0V) < Math.abs(c - this.$N0U) * 0.5
          ) {
            if (c > this.$N0U) {
            ...
            if (
              Math.abs(p - this.$N0V) > j &&
              Math.abs(c - this.$N0U) < Math.abs(p - this.$N0V) * 0.5
            ) {
              if (p > this.$N0V) {
...
bx.CCanvasWnd.drawText = function (
  L,
  y,
  w,
  z,
  ...
  if (f) {
    ...
    if (
      bx.CCanvasWnd.fonts &&
      bx.CCanvasWnd.fonts[Q] &&
      bx.CCanvasWnd.fonts[Q].height !== undefined
    ) {
  ...
  function E(Z, X, W) {
    ...
    switch (j) {
      ...
      case bx.CCanvasWnd.FTW_SPACING_SHRINK:
        if (
          (j === bx.CCanvasWnd.FTW_SPACING_EXPAND && T < z) ||
          (j === bx.CCanvasWnd.FTW_SPACING_SHRINK && T > z)
        ) {
          l = 'left';
          L.textAlign = 'left';
          X = g + d;
            ...
            if (ab == 1) {
              ...
              if (I) {
                ...
                for (; S > 1; S--) {
                  if (/\s|-/.test(ad[ae + S]) || ad.charCodeAt(ae + S) > 255) {
                    if (t(ad.substr(ae, S)) < R) {
                      if (
                        !/\s|-/.test(ad[ae + S]) &&
                        ad.charCodeAt(ae + S + 1) < 255 &&
                        t(ad.substr(ae, S + 1)) < R
                      ) {
                        S++;
                      }
                      aj = true;
                ...
                if (aj) {
                  ...
                  ak = S;
                  while (
                    !(/\s/.test(ad[ae + ak]) || ad.charCodeAt(ae + S) > 255) &&
                    Math.round(t(ad.substr(ae, ak))) > R
                  ) {
                    ak--;
                  }
                  ah = ad.substr(ae, ak);
...
bx.CCanvasWnd.getEnclosingRect = function (l, k, c, p, e, i, h, g) {
  var q;
  if (!(c && p)) {
    return { x1: l, y1: k, x2: l, y2: k };
  }
  if (!e) {
    return { x1: l, y1: k, x2: l + c, y2: k + p };
  }
  e = e % (2 * Math.PI);
  if (e < 0) {
    e = 2 * Math.PI + e;
...
bx.CCanvasWnd.$N2X =
  ' !&amp;&quot;\\#$%&apos;()*+,-./0123456789:;&lt;=&gt;?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[]^_`abcdefghijklmnopqrstuvwxyz{|}~안녕하세요こんにちは早上好你好考试試';
...
bx.CCanvasWnd.loadCustomFont = function (k, g, r, m, j, c, p) {
  ...
  for (h = 0; h < bx.CCanvasWnd._customFontList.length; h++) {
    if (
      bx.CCanvasWnd._customFontList[h].location == g &&
      bx.CCanvasWnd._customFontList[h].fName == k &&
      bx.CCanvasWnd._customFontList[h].loadBold == !!c
    ) {
      console.warn(
        'CCanvasWnd',
        'warning',
        'Already tried fontLocation ' + k + ':' + g,
        'CCanvasWnd.loadCustomFont',
      );
      r(false, k);
      return;
    }
  }
  s = { location: g, fName: k, state: bx.CCanvasWnd.FONT_LOADING, loadBold: !!c };
  bx.CCanvasWnd._customFontList.push(s);
  var e = "'" + k + "'";
  var o = document.createElement('span');
  var q = document.createElement('span');
  var n =
    'position:absolute; left:-10000px; top:-10000px; font-family:serif,sans-serif; white-space:nowrap; font-size:50px; visibility:hidden;';
  var d;
  if (p) {
    ...
    if (!j) {
      ...
      if (g) {
        ...
        if (m) {
          ...
          C.setAttribute(
            'href',
            g + (g.indexOf('?') === -1 ? '?IEFIX=' : '&IEFIX=') + new Date().getTime(),
          );
          ...
          try {
            C.insertRule(
              '@font-face {font-family:' + e + '; src:url(' + g + '); }',
              C.cssRules ? C.cssRules.length : 0,
            );
          } catch (u) {
      ...
      if (g) {
        ...
        if (m) {
          ...
          try {
            D.insertRule(
              '@font-face {font-family:' + e + '; src:url(' + g + '); }',
              D.cssRules ? D.cssRules.length : 0,
            );
          } catch (u) {
    ...
    function E() {
      if (Date.now() > C + z) {
        console.warn(
          'BLUX',
          'warning',
          'bold font loading time out :' + k + ' / ' + g,
          'CCanvasWnd.loadCustomFont',
        );
        y(true);
      } else {
...
bx.CCanvasWnd.intersect_circle_line = function (c, i, h, e, g, d, f) {
  ...
  if (
    (bx.CCanvasWnd.$HuC * bx.CCanvasWnd.$Hu9 - bx.CCanvasWnd.$HuB * bx.CCanvasWnd.$HuA) *
      (bx.CCanvasWnd.$HuC * bx.CCanvasWnd.$Hu9 - bx.CCanvasWnd.$HuB * bx.CCanvasWnd.$HuA) <=
    h * h * (bx.CCanvasWnd.$Hu9 * bx.CCanvasWnd.$Hu9 + bx.CCanvasWnd.$HuA * bx.CCanvasWnd.$HuA)
  ) {
    if (
      bx.CCanvasWnd.$HuB * bx.CCanvasWnd.$HuB + bx.CCanvasWnd.$HuC * bx.CCanvasWnd.$HuC <=
      h * h
    ) {
    ...
    if (
      (bx.CCanvasWnd.$Hu9 - bx.CCanvasWnd.$HuB) * (bx.CCanvasWnd.$Hu9 - bx.CCanvasWnd.$HuB) +
        (bx.CCanvasWnd.$HuA - bx.CCanvasWnd.$HuC) * (bx.CCanvasWnd.$HuA - bx.CCanvasWnd.$HuC) <=
      h * h
    ) {
    ...
    if (
      !bx.CCanvasWnd.$HuD &&
      !bx.CCanvasWnd.$HuE &&
      bx.CCanvasWnd.$HuB * bx.CCanvasWnd.$Hu9 + bx.CCanvasWnd.$HuC * bx.CCanvasWnd.$HuA >= 0 &&
      bx.CCanvasWnd.$HuB * bx.CCanvasWnd.$Hu9 + bx.CCanvasWnd.$HuC * bx.CCanvasWnd.$HuA <=
        bx.CCanvasWnd.$Hu9 * bx.CCanvasWnd.$Hu9 + bx.CCanvasWnd.$HuA * bx.CCanvasWnd.$HuA
    ) {
      return true;
    }
  }
...
bx.CCanvasWnd.setZIndexRange = function (c, d) {
  ...
  if (c.objects && c.objects.length) {
    bx.Trace.log(
      'BLUX',
      'warning',
      'Canvas not empty. It has ' + c.objects.length + ' objects.',
      'CCanvasWnd.setZIndexRange',
    );
  }
  c._slotNumber = d;
  return true;
  ...
  if (l.length == 2) {
    ...
    if (l.length == 3) {
      ...
      for (var g = 0, f = l.length - 1; g < l.length; f = g++) {
        if (
          l[g].y > h != l[f].y > h &&
          k < ((l[f].x - l[g].x) * (h - l[g].y)) / (l[f].y - l[g].y) + l[g].x
        ) {
          e = !e;
        }
      }
...
bx.CCanvasWnd.$5xc = { x: 0, y: 0, w: 0, h: 0 };
bx.CCanvasWnd.$5xd = { x: 0, y: 0, w: 0, h: 0 };
...
bx.CImageEditor.applyToDIV = function (g, f, d) {
  if (!g.imeImage) {
    ...
    g.imeImage = g.$TAG('img', { style: 'position:absolute;' });
  }
...
bx.PageDrag = function (m, q, c, n, p, s, o) {
  ...
  if (window.apn && apn.CExe && m.apnOID) {
    ...
    new bx.CGesture(m, h, { noDelayedEvent: true });
  }
  function w(z) {
    m.bxPageDrag.tagC.$CSS({ transformOrigin: '50% 50% 0' });
    if (m.bxPageDrag.tagLT) {
      m.bxPageDrag.tagLT.$CSS({ transformOrigin: '50% 50% 0' });
    }
    if (m.bxPageDrag.tagRB) {
      m.bxPageDrag.tagRB.$CSS({ transformOrigin: '50% 50% 0' });
    }
...
bx.PageDrag.$4B = function (h, f, i, e, k, c) {
  ...
  if (f == 1) {
    j.styles = { ratio: { from: i, to: -1, unit: g } };
  } else {
    if (f == 2) {
      j.styles = { ratio: { from: i, to: 1, unit: g } };
    }
...
bx.PageDrag.$CQ = function (i, n, e, l, c) {
  ...
  if (g == 1) {
    j.styles = { ratio: { from: k, to: -1, unit: h } };
  } else {
    if (g == 2) {
      j.styles = { ratio: { from: k, to: 1, unit: h } };
    } else {
      j.styles = { ratio: { from: k, to: 0, unit: h } };
    }
  }
  j.onEnd = d;
  if (!m) {
...
bx.PageDrag.$N2R = function (j, l, c, m) {
  function g() {
    j.$CSS({ perspective: parseInt(j.style.width) + 'px', perspectiveOrigin: '50% 50%' });
  ...
  function h() {
    ...
    j.bxPageDrag.tagC.$CSS({ transformStyle: 'flat', transform: 'none' });
    if (j.bxPageDrag.tagLT) {
      j.bxPageDrag.tagLT.$CSS({ transformStyle: 'flat', transform: 'none' });
    }
    if (j.bxPageDrag.tagRB) {
      j.bxPageDrag.tagRB.$CSS({ transformStyle: 'flat', transform: 'none' });
    }
  }
  function f() {
    if (c == 11) {
    ...
    if (j.bxPageDrag.effect == 'cube0') {
      ...
      if (Math.abs(l) > 0.05) {
        ...
        if (l < 0) {
          if (
            j.bxPageDrag.tagRB &&
            !(j.bxPageDrag.tagRB.style.zIndex > j.bxPageDrag.tagC.style.zIndex)
          ) {
            bx.PageDrag.$N1d(j.bxPageDrag.tagRB, j.bxPageDrag.tagC);
          }
          if (j.bxPageDrag.dir == 'x') {
            j.$CSS({ perspectiveOrigin: '100% 50%' });
            j.bxPageDrag.tagC.$CSS({
              transformOrigin: '100% 50%',
            ...
            j.$CSS({ perspectiveOrigin: '50% 100%' });
            j.bxPageDrag.tagC.$CSS({
              transformOrigin: '50% 100%',
          ...
          if (l > 0) {
            if (
              j.bxPageDrag.tagLT &&
              !(j.bxPageDrag.tagLT.style.zIndex > j.bxPageDrag.tagC.style.zIndex)
            ) {
              bx.PageDrag.$N1d(j.bxPageDrag.tagLT, j.bxPageDrag.tagC);
            }
            if (j.bxPageDrag.dir == 'x') {
              j.$CSS({ perspectiveOrigin: '0% 50%' });
              j.bxPageDrag.tagC.$CSS({
                transformOrigin: '0% 50%',
                transform: 'translateX(' + l * 100 + '%) rotateY(' + l * 90 + 'deg)',
              });
              if (j.bxPageDrag.tagLT) {
                j.bxPageDrag.tagLT.$CSS({
                  transformOrigin: '100% 50%',
                  transform:
                    'translateX(' + -(1 - l) * 100 + '%) rotateY(' + -(1 - l) * 90 + 'deg)',
                });
              }
            } else {
              j.$CSS({ perspectiveOrigin: '50% 0%' });
              j.bxPageDrag.tagC.$CSS({
                transformOrigin: '50% 0%',
                transform: 'translateY(' + l * 100 + '%) rotateX(' + -l * 90 + 'deg)',
              });
